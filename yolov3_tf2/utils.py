...
def draw_outputs(img, outputs, class_names):
    ...
    mat=np.array([[ 3.04000000e+00,  5.32000000e+00, -2.12800000e+03],
       [ 0.00000000e+00,  6.62285714e+00,  0.00000000e+00],
       [ 7.50000000e-04,  5.77857143e-03,  1.00000000e+00]])
    pos_dict={}
    for i in range(nums):
    
        x1y1 = tuple((np.array(boxes[i][0:2]) * wh).astype(np.int32))
        x2y2 = tuple((np.array(boxes[i][2:4]) * wh).astype(np.int32))
        x=int((x1y1[0]+x2y2[0])/2)
        y=int((x1y1[1]+x2y2[1])/2)
        if class_names[int(classes[i])] == "person":
        	pos_dict[i]=(x,y)
        	xy=(mat@np.array([x,y,1]))/(mat@np.array([x,y,1]))[2]
        	xy=np.round(xy[:2])
        	white=cv2.circle(white,(int(xy[0]),int(xy[1])),3,(255,0,0),2)
    ...
    for s,i in enumerate(pos_dict.values()):
    	for t,j in enumerate(pos_dict.values()):
    		if s<t:
    			...
    			x2y2=(mat@np.array([j[0],j[1],1]))/(mat@np.array([j[0],j[1],1]))[2]
    			j=np.round(x2y2[:2])
